...
Missing or invalid inputs do not raise. If the requested season is after the last key in the normalization table, `prior_to_elo()` carries the most recent σ forward, prints a warning, and still z-scores the raw prior. If no σ exists at or before that season (the prior does not start until later), or the raw value / σ is null or non-positive, it returns the team's mean-reverted elo — that prior's weight is effectively reallocated to the internal prior for that team. This keeps week 1 projections publishable when upstream data is incomplete, but a broken merge of the raw value still degrades silently.
